# @shapeshift-labs/frontier-canvas-tools

Renderer-neutral editor tools for Frontier canvas surfaces. This package owns grid/editor-style tools, tool controls, tool state machines, permissions, async tool records, transform handles, and AI-operable action manifests without owning rendering.

- npm: [`@shapeshift-labs/frontier-canvas-tools`](https://www.npmjs.com/package/@shapeshift-labs/frontier-canvas-tools)
- source: [`siliconjungle/-shapeshift-labs-frontier-canvas-tools`](https://github.com/siliconjungle/-shapeshift-labs-frontier-canvas-tools)
- license: MIT
...
```sh
npm install @shapeshift-labs/frontier-canvas @shapeshift-labs/frontier-canvas-tools
```

## Usage

```ts
import { createCanvasSurface, createCanvasToolRegistry } from '@shapeshift-labs/frontier-canvas';
import {
...
const surface = createCanvasSurface({
  session: { activeToolId: 'canvasTools.paint' },
  tools
});

const registry = createCanvasToolRegistry({ tools, activeToolId: surface.session.activeToolId });
...
const aiTools = createCanvasToolsAiManifest({ tools, namespace: 'editor' });
```

## Tool Set

- `createPaintCanvasTool` paints grid cells with Bresenham line rasterization between pointer samples.
- `createEraserCanvasTool` erases the same rasterized cell path.
- `createLineCanvasTool` commits a Bresenham line from gesture start to end and can emit preview patches.
- `createRectangleCanvasTool` commits filled or outline rectangles.
- `createFillCanvasTool` uses iterative scanline flood fill over a provided cell snapshot.
- `createEntityBrushCanvasTool` places normalized entity/item records with snap and flip controls.
- `createMoveCanvasTool` and `createResizeCanvasTool` emit transform patches for normalized item maps.
- `materializeCanvasTransformHandles` and `hitTestCanvasHandle` create reusable move/resize handles.

## State Machines

`createCanvasToolMachine`, `selectCanvasTool`, `setCanvasToolMode`, `setCanvasToolControl`, `beginCanvasToolGesture`, `updateCanvasToolGesture`, `commitCanvasToolGesture`, and `cancelCanvasToolGesture` model selected tool, per-mode registries, settings, pointer/gesture capture, hover previews, and commit/cancel phases as patchable state.

Tool machines carry a `stateScope` field. It defaults to `local`, but can be set to `crdt` when an app wants selected tool, mode, settings, or committed gesture state to be shared.

## CRDT And Local State

`createCanvasToolsStateLayout` extends the core canvas state layout with tool-specific paths:

- `/canvas/document/cells`, `/canvas/document/items`, and `/canvas/document/itemsById` default to `crdt`.
- `/canvas/session/toolMachine`, `/canvas/session/preview`, `/canvas/session/activeValue`, and `/canvas/session/toolTasks` default to `local`.

`partitionCanvasToolPatches` uses that layout to split tool output before it reaches a CRDT adapter or local store:

```ts
const layout = createCanvasToolsStateLayout({
  rootPath: '/editor/canvas',
  machineScope: 'crdt'
});

const { crdt, local } = partitionCanvasToolPatches(record.patches, layout);
```

This keeps Inkwell-style document edits syncable while pointer capture, hover previews, active brush settings, and async task UI state stay local unless explicitly promoted.
...
The benchmark covers Bresenham strokes, rectangle cell materialization, scanline fill, tool dispatch, transform handles, tool-machine transitions, and AI manifest generation.

## Boundary

`frontier-canvas-tools` depends on `frontier`, `frontier-canvas`, and `frontier-tools`. It does not import DOM, React, Canvas2D, WebGL, workers, state engines, or app-specific editor runtimes.
